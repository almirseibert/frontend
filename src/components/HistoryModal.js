...
const HistoryModal = ({ vehicle, onClose, obras = [], apiClient }) => {
    ...
    const getHistoryStyle = (type) => {
        switch(type) {
            case 'obra': return 'bg-green-50 border-l-2 border-green-500';
            case 'operacional': return 'bg-blue-50 border-l-2 border-blue-500';
            case 'manutencao': return 'bg-yellow-50 border-l-2 border-yellow-500';
            case 'multa': return 'bg-red-50 border-l-2 border-red-500';
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[80vh] flex flex-col animate-scale-in">
                <div className="p-3 border-b flex justify-between items-center bg-gray-50 rounded-t-lg">
                    <div>
                        ...
                        <p className="text-[10px] text-gray-500">{vehicle.registroInterno} - {vehicle.placa}</p>
                    </div>
                     <button onClick={onClose} className="p-1.5 rounded-full hover:bg-gray-200"><X size={16}/></button>
                </div>
                ...
                <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
                    {loading ? (
                        ...
                        <ul className="space-y-2">
                            {history.map((h, index) => (
                                <li key={h.id || index} className={`p-2 rounded-r-md ${getHistoryStyle(h.historyType || h.type)}`}>
                ...
                <div className="p-3 bg-gray-50 border-t flex justify-end">
                    <button onClick={onClose} className="px-3 py-1.5 bg-gray-200 hover:bg-gray-300 rounded text-xs font-bold text-gray-700">Fechar</button>
                </div>
            </div>
        </div>
    );
