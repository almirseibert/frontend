                            ...
                            return (
                                <tr key={order.id} className="group hover:bg-gray-50 align-middle">
                                    ...
                                    <td className="px-2 py-2.5 text-right font-medium text-gray-900 whitespace-nowrap">
                                        {order.status === 'Pendente de Valor' ? 'A Cotar' : fmtBRL(order.totalValue)}
                                    </td>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div><p className="text-xs text-blue-600 font-bold uppercase mb-1">Valor Total Autorizado</p><p className="font-black text-blue-900 text-lg">{fmtBRL(order.totalValue)}</p>{payment.discount?.amount > 0 && <p className="text-[11px] text-blue-700">Subtotal {fmtBRL(payment.discount.subtotal)} − desconto {payment.discount.type === 'percent' ? `${payment.discount.value}% ` : ''}{fmtBRL(payment.discount.amount)}</p>}</div>
                        </div>
                            ...
                            <div className="flex-1 border-t lg:border-t-0 pt-4 lg:pt-0">
                                ...
                                {!isPricePending && (
                                    <div className="mt-4 border-t pt-3 flex flex-wrap items-center justify-end gap-2 text-xs">
                                        <span className="font-bold text-gray-500 uppercase mr-auto">Subtotal: {fmtBRL(subtotalValue)}</span>
                                        ...
                                        <input
                                            ...
                                            disabled={isReadOnly}
                                        />
                                        {discountAmount > 0 && <span className="font-bold text-red-600">- {fmtBRL(discountAmount)}</span>}
                                    </div>
                                )}
